...
prof_vertex_colors[prof_vertex_colors == 6] <- colors[6]
...
legend('topleft',fill=c('Lightblue', 'Red','Green','Yellow','Orange','Purple'), legend=c('Nurse','Doctor','Allied HP','Social services','Manager','Other'))
```
...
legend('topleft',fill=c('Lightblue', 'Red','Green','Yellow','Orange','Purple'), legend=levels(prof$Profession)[1:6])
...
***** 
_Note: this is preliminary as we don't have complete data on professional backgrounds_
****
...
e1 <- ergm(adv.net ~  edges  + odegree(0) + mutual + nodeifactor('Profession',base=c(3,4,6)) + nodematch('Profession', diff=TRUE, keep=c(1,2,5))  ,control=control.ergm(seed=10, MCMLE.maxit=50))
...
One way to  investigate whether there is evidence of assortative matching controling for the frequency of different professional groups in the network is by means of an exponential random graph model.  The table below shows the result sof an estimate that includes main effects of professional background as well as variables that measure the propensity of people in the three main professional groups to select members of their own profession.
...
e2 <- ergm(lead.net ~  edges  + odegree(0) + mutual + nodeifactor('Profession',base=c(3,4,6)) + nodematch('Profession', diff=TRUE, keep=c(1,2,5))  ,control=control.ergm(seed=10, MCMLE.maxit=50))
...
Mutual ties      |`r b.e2[3]`* | `r se.e2[3]`
Nurses           |`r b.e2[4]`* | `r se.e2[4]`
Doctors          |`r b.e2[5]`* | `r se.e2[5]`
Managers         |`r b.e2[6]`  | `r se.e2[6]`
Nurse homophily  |`r b.e2[7]`  | `r se.e2[7]`
Doctor homophily |`r b.e2[8]`  | `r se.e2[8]`
Manager homophily|`r b.e2[9]`  | `r se.e2[9]`  
Note: * = *p* < .05
...
plot(infl.noiso, layout=layout.kamada.kawai,edge.arrow.size=.8,
     vertex.label=NA,vertex.color=prof_vertex_colors,vertex.size=inf.deg, main="Leadership network")
legend('topleft',fill=c('Lightblue', 'Red','Green','Yellow','Orange','Purple'), legend=c('Nurse','Doctor','Allied HP','Social services','Manager','Other'))
```
...
e3 <- ergm(infl.net ~  edges  + odegree(0) + nodeifactor('Profession',base=c(3,4,6)) + nodematch('Profession', diff=TRUE, keep=c(1,2,5))  ,control=control.ergm(seed=10, MCMLE.maxit=50))
...
```{r advicegof, message=FALSE, cache=TRUE}
library(network)
library(ergm)
detach(package:igraph)
gof.advice <- gof(e1)
